...
class Curse:
    ...
    def worker(self, ph):
        ...
        while not self.stop_event.is_set():
            ...
            if not p_dict:
                print(f"{WHITE}[{CYAN}!{WHITE}]{RESET} Waiting for proxies...")
                time.sleep(5)
                continue
    ...
    def start(self):
        os.system('cls')
        print(f"{CYAN}Engine starting on Windows...{RESET}")
        
        proxy_path = 'proxies.txt'
        if not os.path.exists(proxy_path) or os.stat(proxy_path).st_size == 0:
            print(f"{WHITE}[{CYAN}!{WHITE}]{RESET} Error: proxies.txt is empty or missing!")
            return

        with open(proxy_path, 'r') as f: pxs = f.readlines()
        print(f"{CYAN}Loaded {len(pxs)} proxies.{RESET}")
        ...
        try:
            while True:
                ...
                title = f"Requests = {self.stats['total']} | RPS > {self.stats['rps']}"
